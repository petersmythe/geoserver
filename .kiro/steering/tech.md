...
### Current Approach
- Hand-built OpenAPI 3.0 spec generated by Python scripts in `.kiro/api-analysis/`
- Modular source specs in `.kiro/api-analysis/specs/` (REST, OGC, common components)
- Bundled output at `doc/en/api/geoserver-bundled.yaml` and `.json`
- Bundler script: `.kiro/api-analysis/bundle-spec.py` (run from `.kiro/api-analysis/` with venv active)

### Future Approach (Phase 3)
- Springdoc OpenAPI annotations on Java controllers (planned, not yet implemented)
- Auto-generated spec from code at build time or runtime
- When adding new REST endpoints, structure code to be annotation-friendly:
  - Use typed request/response objects (not raw Map/String)
  - Keep path patterns simple and OpenAPI-compatible
  - Avoid Spring-specific path regex patterns (e.g., `{ext:xml|json}`) where possible
  - Use descriptive path variable names matching the `@PathVariable` annotation
